...
import { usePromotedOffers } from '@/hooks/useData';
import type { PromotedOffer, Provider } from '@/data/mockData';
...
const PromoBanner = () => {
  ...
  const { data: items = [] } = usePromotedOffers();
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <div className="flex">
          {items.map(({ promo, provider }) => (
            <PromoBannerCard key={promo.id} promo={promo} provider={provider} />
          ))}
        </div>
      </div>
...
const PromoBannerCard = ({ promo, provider }: { promo: PromotedOffer; provider: Provider }) => (
  <Link to={`/offer/${promo.offerId}`} className="flex-none w-full">
    <div
      className={cn(
